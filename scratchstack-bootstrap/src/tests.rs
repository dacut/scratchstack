...
async fn test_migrate_database(database: &TempDatabase) {
    ...
    let result = database
        .run(["ssbs", "--port", &port, "--username", "scratchstack", "db", "migrate"])
        .await
...
async fn test_partition(database: &TempDatabase) {
    ...
    let result = database
        .run(["ssbs", "--port", &port, "--username", "scratchstack", "iam", "get-current-partition"])
        .await
...
async fn test_accounts(database: &TempDatabase) {
    ...
    let result = database
        .run(["ssbs", "--port", &port, "--username", "scratchstack", "iam", "create-account"])
        .await
    ...
    let mut result = database
        .run(["ssbs", "--port", &port, "--username", "scratchstack", "iam", "list-accounts"])
        .await
...
async fn test_users(database: &TempDatabase) {
    ...
    database
        .run(["ssbs", "--port", &port, "--username", "scratchstack", "iam", "delete-policy", "--policy-arn", pb_arn])
        .await
...
async fn test_groups(database: &TempDatabase) {
    ...
    assert!(!err.code().is_empty(), "Expected an error code, got: {err}");
...
async fn test_session_token_encryption_keys(database: &TempDatabase) {
    ...
    let result = database
        .run(["ssbs", "--port", &port, "--username", "scratchstack", "iam", "list-session-token-encryption-keys"])
        .await
