...
pub async fn list_group_policies(
    ...
    let rows = sql
        .build_query_as::<ListGroupPoliciesRow>()
        .fetch_all(tx.as_mut())
        .await
        .map_err(|e| internal_failure!(request_id; "Failed to fetch group inline policies from database: {e}"))?;
        ...
        if results.len() == max_items {
            next_marker = Some(
                paginator
                    ...
                    .await
                    .map_err(|e| {
                        internal_failure!(request_id; "Failed to encrypt pagination token for ListGroupPolicies: {e}")
                    })?,
            );
            break;
        }
    ...
    builder.build().map_err(|e| internal_failure!(request_id; "Failed to build ListGroupPoliciesResponse: {e}").into())
}
