...
use {
    crate::{
        ...
        group::validate_group_name, iam_internal_failure, make_iam_paginator, partition::get_current_partition_or_fail,
    },
    indoc::indoc,
...
impl RequestExecutor for ListGroupPoliciesInternalRequest {
    type Response = ListGroupPoliciesResponse;
    type Error = IamError;

    async fn execute(&self, tx: &mut PgTransaction<'_>, request_id: RequestId) -> Result<Self::Response, Self::Error> {
        list_group_policies(tx, &self.account_id, &self.group_name, self.marker.as_deref(), self.max_items, request_id)
            .await
    }
}

/// The marker innards for a ListGroupPolicies operation.
#[derive(Deserialize, Serialize)]
struct ListGroupPoliciesMarker {
    next_policy_name_lower: String,
}

/// The rows returned by the ListGroupPolicies query.
#[derive(FromRow)]
struct ListGroupPoliciesRow {
    policy_name_lower: String,
    policy_name_cased: String,
}

/// List the names of inline policies attached to a group. Returns NoSuchEntity if the group does
/// not exist.
pub async fn list_group_policies(
...
) -> Result<ListGroupPoliciesResponse, IamError> {
    ...
    let paginator = make_iam_paginator(&partition, OP_LIST_GROUP_POLICIES, request_id)?;
    ...
    let rows =
        sql.build_query_as::<ListGroupPoliciesRow>().fetch_all(tx.as_mut()).await.map_err(
            |e| iam_internal_failure!(request_id; "Failed to fetch group inline policies from database: {e}"),
        )?;
    ...
    for row in rows.into_iter() {
        if results.len() == max_items {
            next_marker = Some(
                paginator
                    .encrypt_token(&ListGroupPoliciesMarker {
                        next_policy_name_lower: row.policy_name_lower,
                    })
                    .await
                    .map_err(|e| {
                        iam_internal_failure!(request_id; "Failed to encrypt pagination token for ListGroupPolicies: {e}")
                    })?,
            );
            break;
        }
    ...
    builder
        .build()
        .map_err(|e| iam_internal_failure!(request_id; "Failed to build ListGroupPoliciesResponse: {e}").into())
}
