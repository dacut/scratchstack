...
pub fn validate_user_name(user_name: impl AsRef<str>, request_id: RequestId) -> Result<(), ValidationError> {
    const MESSAGE: &str = "User name must contain only alphanumeric characters or the following symbols: +=,.@-_ and must be between 1 and 64 characters long.";
...
mod tests {
    ...
    #[test_log::test]
    fn user_name_message_lists_every_accepted_symbol() {
        validate_user_name(format!("Name{IAM_RESOURCE_NAME_SYMBOLS}"), RequestId::new())
            .expect("every symbol the message lists must be accepted");

        let err = validate_user_name("has a space", RequestId::new()).expect_err("a space is not accepted");
    ...
    #[test_log::test]
    fn user_name_length_bound_matches_the_message() {
        validate_user_name("a".repeat(64), RequestId::new()).expect("the documented maximum must be accepted");
        validate_user_name("a".repeat(64 + 1), RequestId::new()).expect_err("one over the maximum must be rejected");
        validate_user_name("", RequestId::new()).expect_err("an empty name must be rejected");
    }
}
