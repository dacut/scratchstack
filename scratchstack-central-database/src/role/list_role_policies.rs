...
use {
    crate::{
        ...
        iam_internal_failure, make_iam_paginator, partition::get_current_partition_or_fail, role::validate_role_name,
    },
    indoc::indoc,
...
impl RequestExecutor for ListRolePoliciesInternalRequest {
    ...
    async fn execute(&self, tx: &mut PgTransaction<'_>, request_id: RequestId) -> Result<Self::Response, Self::Error> {
        list_role_policies(tx, &self.account_id, &self.role_name, self.marker.as_deref(), self.max_items, request_id)
            .await
    }
}
...
pub async fn list_role_policies(
    ...
    let paginator = make_iam_paginator(&partition, OP_LIST_ROLE_POLICIES, request_id)?;
    ...
    let rows =
        sql.build_query_as::<ListRolePoliciesRow>().fetch_all(tx.as_mut()).await.map_err(
            |e| iam_internal_failure!(request_id; "Failed to fetch role inline policies from database: {e}"),
        )?;
        ...
        if results.len() == max_items {
            next_marker = Some(
                paginator
                    ...
                    .await
                    .map_err(|e| {
                        iam_internal_failure!(request_id; "Failed to encrypt pagination token for ListRolePolicies: {e}")
                    })?,
            );
            break;
        }
    ...
    builder
        .build()
        .map_err(|e| iam_internal_failure!(request_id; "Failed to build ListRolePoliciesResponse: {e}").into())
}
