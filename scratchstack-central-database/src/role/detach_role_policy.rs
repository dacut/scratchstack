...
impl RequestExecutor for DetachRolePolicyInternalRequest {
    ...
    async fn execute(&self, tx: &mut PgTransaction<'_>, request_id: RequestId) -> Result<Self::Response, Self::Error> {
        detach_role_policy(tx, &self.account_id, &self.role_name, &self.policy_arn, request_id).await
    }
}
...
pub async fn detach_role_policy(
    ...
    .await
    {
        Ok(result) => result,
        Err(e) => {
            return Err(iam_internal_failure!(request_id; "Failed to detach policy from role in database: {e}").into());
        }
    };
