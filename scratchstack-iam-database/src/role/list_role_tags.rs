...
pub async fn list_role_tags(
    ...
    .await
    .map_err(|e| {
        error!("Failed to check if role exists in database: {e}");
        internal_failure()
    })?;
    ...
    if let Some(marker) = marker {
        let m: ListRoleTagsMarker = paginator.decrypt_token(marker).await.map_err(|e| {
            error!("Failed to decrypt pagination token for ListRoleTags: {e}");
            internal_failure()
        })?;
        sql.push("\nAND key_lower >= ");
        ...
        if results.len() == max_items {
            next_marker = Some(
                paginator
                    ...
                    .await
                    .map_err(|e| {
                        error!("Failed to encrypt pagination token for ListRoleTags: {e}");
                        internal_failure()
                    })?,
            );
            break;
    ...
    Ok(ListRoleTagsResponse::builder()
        .set_tags(results)
        .is_truncated(next_marker.is_some())
        .set_marker(next_marker)
        .build()
        .map_err(|e| {
            error!("Failed to build ListRoleTagsResponse: {e}");
            internal_failure()
        })?)
}
