...
macro_rules! impl_error {
    ...
    ( $(#[$outer:meta])* struct $name:ident branch $branch:ident code $code:ident status $sc:ident kind $kind:ident message $msg:expr ; $($rest:tt)* ) => {
        ...
        pub struct $name {
            ...
            #[builder(into, default = $msg.to_string())]
            message: String,

            /// The request id associated with this operation, if available.
            #[builder(into)]
...
/// A trait for mapping [`SignatureError`] values to HTTP responses.
pub trait SignatureErrorMapper {
    /// Map a [`SignatureError`] to an HTTP response.
    fn map_error(&self, error: SignatureError) -> Response<Body>;
}

/// An implementation of [`SignatureErrorMapper`] that maps errors to XML responses.
#[derive(Builder, Clone, Debug)]
pub struct SignatureErrorToXmlResponse {
    /// The XML namespace of the service.
    #[builder(into)]
    pub xmlns: String,
}

impl SignatureErrorMapper for SignatureErrorToXmlResponse {
    fn map_error(&self, error: SignatureError) -> Response<Body> {
        ErrorResponseEnvelope::new_with_xmlns(&error, &self.xmlns).respond()
    }
}
...
impl From<KeyLengthError> for SignatureError {
    fn from(e: KeyLengthError) -> Self {
        error!("SignatureError created from KeyLengthError (and will lose information): {e}");
        InternalFailureError::default().into()
    }
}

/// Convenience function for returning an InvalidClientTokenIdError with the message
/// `"The security token included in the request is invalid"`.
#[inline(always)]
pub(crate) fn invalid_session_token_error() -> SignatureError {
    InvalidClientTokenIdError::builder().message(ERR_MSG_INVALID_SECURITY_TOKEN).build().into()
}

#[cfg(test)]
mod tests {
    use {super::*, scratchstack_core::http::StatusCode, std::error::Error};
    ...
    fn test_from() {
        ...
        let e: SignatureError = MalformedQueryStringError::builder().message("foo").build().into();
        let e2 = SignatureError::from(Box::new(e) as Box<dyn Error + Send + Sync + 'static>);
        assert_eq!(e2.to_string(), "foo");
        assert_eq!(e2.code(), "MalformedQueryString");

        let e: SignatureError =
            InvalidContentTypeError::builder().message("Invalid content type: image/jpeg").build().into();
        assert_eq!(e.code(), "InvalidContentType");
        assert_eq!(e.http_status(), Some(StatusCode::FORBIDDEN)); // Should be 400, but AWS returns 403.
        assert_eq!(format!("{}", e), "Invalid content type: image/jpeg");

        let e: SignatureError =
            InvalidRequestMethodError::builder().message("Invalid request method: DELETE").build().into();
        assert_eq!(e.code(), "InvalidRequestMethod");
        assert_eq!(e.http_status(), Some(StatusCode::METHOD_NOT_ALLOWED));
