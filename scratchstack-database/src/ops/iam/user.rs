...
use {
    crate::{
        ...
        ops::{
            RequestExecutor,
            iam::{
                constrain_max_items, get_current_partition_or_fail, get_permissions_boundary_id, parse_policy_arn,
                validate_account_id, validate_path, validate_path_prefix, validate_tag_key, validate_tag_value,
                validate_user_name,
            },
        },
    },
...
impl RequestExecutor for AttachUserPolicyInternalRequest {
    ...
    async fn execute(&self, tx: &mut PgTransaction<'_>) -> Result<Self::Response, Self::Error> {
        attach_user_policy(tx, &self.account_id, &self.user_name, &self.policy_arn).await
    }
}
