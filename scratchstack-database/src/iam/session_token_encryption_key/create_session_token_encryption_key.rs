...
use {
    crate::{IamId, RequestExecutor, constants::iam::*},
...
pub async fn create_session_token_encryption_key(
    ...
    let accept_expires_at = match accept_expires_at {
        None => {
            issue_expires_at
                + Duration::seconds(MAX_SESSION_TOKEN_DURATION_SECS + MAX_SESSION_TOKEN_TIMESTAMP_ERROR_SECS)
        }
    ...
    let mut encryption_key: [u8; AES256_KEY_SIZE_BYTES] = [0; AES256_KEY_SIZE_BYTES];
