...
use {
    crate::{
        RequestExecutor,
        constants::iam::*,
        iam::{get_permissions_boundary_id, validate_account_id, validate_role_name},
    },
    indoc::indoc,
    scratchstack_shapes_iam::{
        error_meta::Error as IamError,
        operation::PutRolePermissionsBoundaryInternalRequest,
        types::error::{InternalFailure, NoSuchEntityException},
    },
    sqlx::{postgres::PgTransaction, query},
};

impl RequestExecutor for PutRolePermissionsBoundaryInternalRequest {
    type Response = ();
    type Error = IamError;
...
/// Set the permissions boundary on a role to the managed policy identified by the supplied ARN.
/// Replaces any previously-set boundary. Returns NoSuchEntityException if either the role or the
/// permissions boundary policy is missing.
pub async fn put_role_permissions_boundary(
