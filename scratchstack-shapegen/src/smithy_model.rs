...
impl SmithyModel {
    ...
    pub fn resolve_self(&mut self) {
        ...
        let xmlns = service.traits.xml_namespace().unwrap().to_string();
        drop(service_shape);
    ...
    fn generate_error_meta<W: Write>(&self, w: &mut Writers<W>) -> IoResult<()> {
        ...
        writeln!(w.error_meta, "    /// Returns this error as a `dyn ErrorMetadata` reference")?;
        writeln!(
            w.error_meta,
        ...
        for shape in self.shapes.values() {
                ...
                && s.base.traits.is_error()
            {
                writeln!(
                    w.error_meta,
                    "            Self::{}(inner) => inner as &dyn ::scratchstack_core::error::ProvideErrorMetadata,",
                    s.base.rust_typename()
                )?;
            }
        }
        writeln!(
            w.error_meta,
            "            Self::Unhandled(inner) => inner as &dyn ::scratchstack_core::error::ProvideErrorMetadata,"
        )?;
        writeln!(w.error_meta, "        }}")?;
        writeln!(w.error_meta, "    }}")?;
        writeln!(w.error_meta, "}}")?;
