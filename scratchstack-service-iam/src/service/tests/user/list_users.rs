...
/// database. A single test function covers every case so that they share one seeded account,
/// rather than seeding one apiece.
...
async fn test_list_users_authorization() {
    ...
    let (principal, session_data) = database.user_identity("SVCTESTALLOWUSER", "Allowed-User");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTDENYUSER1", "Denied-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    assert!(
        body.contains(&format!(
            "User: arn:aws:iam::{account_id}:user/Denied-User is not authorized to perform: \
    ...
    let (principal, session_data) = database.user_identity("SVCTESTTLSUSER01", "Tls-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTTIMEUSER1", "Time-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");

    let (principal, session_data) = database.user_identity("SVCTESTPASTUSER1", "Past-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTEPOCHUSR1", "Epoch-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTIPV4USER1", "Ipv4-User");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTIPV6USER1", "Ipv6-User");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTDENYIPUSR", "Deny-Ip-User");
    let (status, body) = call_from(&svc_state, principal, session_data, TEST_SOURCE_IP, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTREGIONUSR", "Region-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");

    let (principal, session_data) = database.user_identity("SVCTESTOTHERRGN1", "Other-Region-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.role_identity("SVCTESTRGNROLE01", "Region-Role");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTACCTUSER1", "Account-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");

    let (principal, session_data) = database.user_identity("SVCTESTOTHRACCT1", "Other-Account-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTAGENTUSR1", "Agent-User");
    ...
    // sessions.
    let (principal, session_data) = database.user_identity("SVCTESTDIRECTUSR", "Direct-Call-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");

    let (principal, session_data) = database.role_identity("SVCTESTDIRROLE01", "Direct-Call-Role");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.role_identity("SVCTESTTOKENROLE", "Token-Role");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.role_identity_issued_at("SVCTESTTOKENROLE", "Token-Role", issued_at);
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCTESTTOKENUSER", "Token-User");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.role_identity("SVCTESTROLE00001", "session-role");
    let (status, body) = call(&svc_state, principal, session_data, parameters).await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    assert!(body.contains("<ListUsersResult>"), "unexpected body: {body}");
    ...
    assert!(
        body.contains(&format!(
            "User: arn:aws:sts::{account_id}:assumed-role/session-role/test-session is not \
