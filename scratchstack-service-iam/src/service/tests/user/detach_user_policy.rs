...
const DETACH_USER_POLICY_TEST_DATA: &str = r#"
    ...
    ('%ACCOUNT_ID%', 'detach-user-policy-test@example.com', 'detach-user-policy-test');

    INSERT INTO iam.users(user_id, account_id, user_name_lower, user_name_cased, path) VALUES
    ('SVCDUPBROADDET01', '%ACCOUNT_ID%', 'broad-detacher', 'Broad-Detacher', '/'),
    ('SVCDUPSAFEDET001', '%ACCOUNT_ID%', 'safe-detacher', 'Safe-Detacher', '/'),
    ('SVCDUPPATHDET001', '%ACCOUNT_ID%', 'path-detacher', 'Path-Detacher', '/'),
    ('SVCDUPTAGDET0001', '%ACCOUNT_ID%', 'tag-detacher', 'Tag-Detacher', '/'),
    ('SVCDUPNARROWD001', '%ACCOUNT_ID%', 'narrow-detacher', 'Narrow-Detacher', '/'),
    ('SVCDUPNOGRANTD01', '%ACCOUNT_ID%', 'no-grant-detacher', 'No-Grant-Detacher', '/'),
    ('SVCDUPTGTPLAIN01', '%ACCOUNT_ID%', 'detach-target', 'Detach-Target', '/'),
    ('SVCDTPTGTDIVSN01', '%ACCOUNT_ID%', 'division-target', 'Division-Target', '/division/'),
    ('SVCDTPTGTENGNR01', '%ACCOUNT_ID%', 'engineering-target', 'Engineering-Target', '/'),
    ('SVCDTPTGTSALES01', '%ACCOUNT_ID%', 'sales-target', 'Sales-Target', '/'),
    ('SVCDUPTGTROLE001', '%ACCOUNT_ID%', 'role-target', 'Role-Target', '/'),
    ('SVCDTPTGTROOT001', '%ACCOUNT_ID%', 'root-target', 'Root-Target', '/');

    INSERT INTO iam.user_tags(user_id, key_lower, key_cased, value) VALUES
    ('SVCDTPTGTENGNR01', 'department', 'Department', 'Engineering'),
    ('SVCDTPTGTSALES01', 'department', 'Department', 'Sales');
    ...
    ('SVCDUPPOLSAFE001', '%ACCOUNT_ID%', 'safe-policy', 'Safe-Policy', '/safe/', 1, false, 1),
    ('SVCDUPPOLADMIN01', '%ACCOUNT_ID%', 'admin-policy', 'Admin-Policy', '/', 1, false, 1),
    ('SVCDUPPOLEXTRA01', '%ACCOUNT_ID%', 'extra-policy', 'Extra-Policy', '/', 1, false, 1);
    ...
    ('SVCDTPTGTDIVSN01', 'SVCDUPPOLADMIN01'),
    ('SVCDTPTGTENGNR01', 'SVCDUPPOLADMIN01'),
    ('SVCDTPTGTSALES01', 'SVCDUPPOLADMIN01'),
    ('SVCDUPTGTROLE001', 'SVCDUPPOLADMIN01'),
    ('SVCDTPTGTROOT001', 'SVCDUPPOLADMIN01');
    ...
    ('SVCDUPNARROWD001', 'allow-detach-from-target', 'Allow-Detach-From-Target',
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":"iam:DetachUserPolicy",
        "Resource":"arn:aws:iam::%ACCOUNT_ID%:user/Detach-Target"}]}');

    INSERT INTO iam.roles(role_id, account_id, role_name_lower, role_name_cased, path, assume_role_policy_document) VALUES
    ('SVCDUPROLE000001', '%ACCOUNT_ID%', 'detach-user-policy-role', 'Detach-User-Policy-Role', '/',
...
async fn test_detach_user_policy_authorization() {
    ...
    let (principal, session_data) = database.user_identity("SVCDUPSAFEDET001", "Safe-Detacher");
    let (status, body) = call(
        &svc_state,
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Detach-Target"), Some(safe_policy_arn.as_str())),
    )
    .await;
    ...
    let (status, body) = call(
        ...
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Detach-Target"), Some(admin_policy_arn.as_str())),
    )
    .await;
    ...
    let (principal, session_data) = database.user_identity("SVCDUPBROADDET01", "Broad-Detacher");
    let (status, body) = call(
        &svc_state,
        principal,
        session_data,
    ...
    let (principal, session_data) = database.user_identity("SVCDUPPATHDET001", "Path-Detacher");
    let (status, body) = call(
        &svc_state,
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Division-Target"), Some(admin_policy_arn.as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    ...
    let (status, body) = call(
        ...
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Detach-Target"), Some(extra_policy_arn.as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCDUPTAGDET0001", "Tag-Detacher");
    let (status, body) = call(
        &svc_state,
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Engineering-Target"), Some(admin_policy_arn.as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    ...
    let (status, body) = call(
        ...
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Sales-Target"), Some(admin_policy_arn.as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::FORBIDDEN, "unexpected response: {body}");
    assert!(body.contains("<Code>AccessDenied</Code>"), "unexpected body: {body}");
    ...
    let (principal, session_data) = database.user_identity("SVCDUPNARROWD001", "Narrow-Detacher");
    let (status, body) = call(
        &svc_state,
        principal,
        session_data,
    ...
    let (principal, session_data) = database.user_identity("SVCDUPNOGRANTD01", "No-Grant-Detacher");
    let (status, body) = call(
        &svc_state,
        principal,
        session_data,
    ...
    assert!(
        body.contains(&format!(
            "User: arn:aws:iam::{account_id}:user/No-Grant-Detacher is not authorized to perform: \
                 iam:DetachUserPolicy on resource: arn:aws:iam::{account_id}:user/Sales-Target"
        )),
        "unexpected body: {body}"
    );
    ...
    let (status, body) = call(
        ...
        principal,
        session_data,
        &detach_user_policy_parameters(Some("No-Such-User"), Some(admin_policy_arn.as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::NOT_FOUND, "unexpected response: {body}");
    assert!(body.contains("<Code>NoSuchEntity</Code>"), "unexpected body: {body}");
    ...
    let (status, body) = call(
        ...
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Sales-Target"), Some(database.arn("policy/No-Such-Policy").as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::NOT_FOUND, "unexpected response: {body}");
    assert!(body.contains("<Code>NoSuchEntity</Code>"), "unexpected body: {body}");
    ...
    for parameters in [
        detach_user_policy_parameters(None, Some(admin_policy_arn.as_str())),
    ...
    let (principal, session_data) = database.role_identity("SVCDUPROLE000001", "Detach-User-Policy-Role");
    let (status, body) = call(
        &svc_state,
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Role-Target"), Some(admin_policy_arn.as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
    ...
    let (status, body) = call(
        ...
        principal,
        session_data,
        &detach_user_policy_parameters(Some("Root-Target"), Some(admin_policy_arn.as_str())),
    )
    .await;
    assert_eq!(status, StatusCode::OK, "unexpected response: {body}");
}
