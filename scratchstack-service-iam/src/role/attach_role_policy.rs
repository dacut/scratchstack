use {
    ...
    scratchstack_shapes_iam::{
        action::Action,
        operation::{AttachRolePolicyInternalRequest, AttachRolePolicyRequest, AttachRolePolicyResponseEnvelope},
    },
};

/// Handle an `AttachRolePolicy` request.
///
/// The caller has already been authenticated by the SigV4 layer. The caller's identity-based
/// policies (including any permissions boundary), intersected with any session policies, must
/// allow `iam:AttachRolePolicy` on the role the policy is attached to; the account root user is
/// implicitly allowed.
///
/// A managed policy is a resource of its own, unlike the inline policies
/// [`crate::role::put_role_policy`] writes, but it is not the resource this action acts on: IAM
/// gives `iam:AttachRolePolicy` the role as its resource type, so the action is authorized
/// against the role's ARN alone. The policy being attached is named by the `iam:PolicyARN`
/// condition key instead, which is what lets a grant say which policies a caller may hand out
/// without saying anything about which roles may receive them, or the reverse.
///
/// Both halves matter, because attaching a managed policy grants its permissions to the role:
/// `iam:AttachRolePolicy` on a role is a privilege escalation unless it is confined by
/// `iam:PolicyARN` to policies no more privileged than the caller, and the permissions land with
/// anyone who can assume the role rather than with the caller alone. Nothing here checks that the
...
/// itself is idempotent, so attaching a policy the role already carries succeeds and changes
/// nothing.
///
...
pub(crate) async fn attach_role_policy(
    ...
    let request: AttachRolePolicyRequest = match from_query_str(parameters) {
        Ok(request) => request,
        Err(e) => {
            log::debug!("{request_id}: Could not parse AttachRolePolicy parameters: {e}");
    ...
    let request = match AttachRolePolicyInternalRequest::builder()
    ...
    // The policy being attached and the role receiving it are distinct facts, exposed through
    ...
    // The attachment reports a role or a policy that does not exist as `NoSuchEntity` itself, so
    // neither missing case needs separate handling here.
    let response = match request.execute(&mut tx, request_id).await {
        Ok(()) => AttachRolePolicyResponseEnvelope::builder().request_id(request_id).build().respond(),
