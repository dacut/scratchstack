use {
    ...
    scratchstack_shapes_iam::{
        action::Action,
        operation::{TagRoleInternalRequest, TagRoleRequest, TagRoleResponseEnvelope},
    },
};

/// Handle a `TagRole` request.
///
/// The caller has already been authenticated by the SigV4 layer. The caller's identity-based
/// policies (including any permissions boundary), intersected with any session policies, must
/// allow `iam:TagRole` on the role being tagged; the account root user is implicitly allowed.
///
...
/// tags a caller may write and what it may write in them; the tags the role already carries back
/// `aws:ResourceTag/${TagKey}` and `iam:ResourceTag/${TagKey}`, so a policy can limit which roles
/// a caller may tag at all. A caller allowed to add a tag to a role is thereby allowed to change
/// the tags it is already carrying, which is what makes `iam:TagRole` worth conditioning: a role
...
///
/// `RoleName` and at least one tag are both required. A tag whose key is already on the role
...
pub(crate) async fn tag_role(
    ...
    let request: TagRoleRequest = match from_query_str(parameters) {
        Ok(request) => request,
        Err(e) => {
            log::debug!("{request_id}: Could not parse TagRole parameters: {e}");
            return malformed_input(request_id);
        }
    };
    let role_name = request.role_name;

    // Building the internal request validates the role name and the shape of the tag list, so a
    ...
    // than that.
    let request = match TagRoleInternalRequest::builder()
    ...
    // The tags the request asks to apply and the ones the role already carries are distinct
    // facts, exposed through distinct condition keys, so both are supplied: a policy can be
    // conditioned on what is being written, on what the role already holds, or on both at once.
    ...
    let response = match request.execute(&mut tx, request_id).await {
        Ok(()) => TagRoleResponseEnvelope::builder().request_id(request_id).build().respond(),
